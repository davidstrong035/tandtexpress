...
router.post("/transit", function(req, res){ console.log(req.body);
		...
		.then(data=> {
			if(data.length == 0){
				...
				userModel.where({"parcel.tracking": req.body.tracking})
					.update({$set: {"progress.transit": status}}, function(err, count){
						if(err) {
							res.stus(400).send({"message": "Couldn't update the transit status"});
						} else {
							res.status(200).send({"message": "Package is now on transit"});
						}
					});
			}
		})
